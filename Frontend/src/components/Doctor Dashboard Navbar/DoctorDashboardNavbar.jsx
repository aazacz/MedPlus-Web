...
function DoctorDashboardNavbar(props) {
    ...
    return (
        ...
        <>
            <div className={`relative  md:mt-0 md:w-full md:h-[40px] md:items-center  ${!toggle ? "w-[50px]" : "w-[250px]"}  bg-lightBlue-800 flex justify-end md:pr-5 transition-all ease-in duration-150`}>
                <ul className='   md:h-[35px] md:flex md:gap-3'>
                        ...
                        <Link to={`${val.link}`} key={key}>
                            <li className={` my-4 w-full  h-[60px] md:h-[35px] md:my-0`} >
                                ...
                                <div className={` h-full flex flex-row items-center md:w-max  md:px-3  md:rounded-none         
                                    ...
                                    <div className={`${location.pathname === val.link ? 'text-white rounded-l-full active' : 'text-black'} ml-3 md:ml-0 md:hidden `}>{val.icon}</div>
                                    <div data-appear="hidemenu" className={`md:text-white  ${menuClass} ${location.pathname === val.link ? 'text-white rounded-l-full active' : 'text-black'} md:ml-0 ml-[10px] text-sm `}>{toggle?val.title:""}</div>
